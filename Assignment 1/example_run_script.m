...
for ramp_angle = 6.5
    ...
    % stepsCount = countsteps(initial_inter_leg_angle,inter_leg_angles,time)

    % Save results
    results = [results; ramp_angle, steps];
...
for i = 1:size(results, 1)
    if results(i, 2) >= 5
        plot(results(i, 1), results(i, 2), 'ro', 'MarkerSize', 8, 'LineWidth', 1.5); % Red marker for > 5 steps
    else
        plot(results(i, 1), results(i, 2), 'bo', 'MarkerSize', 6, 'LineWidth', 1.5); % Blue marker for ≤ 5 steps
    end
end
xlabel('Ramp Angle (degrees)');
ylabel('Number of Steps');
